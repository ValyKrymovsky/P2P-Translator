...
class Server():
    ...
    def get_program(self, word: str):
        host_ip = socket.gethostbyaddr(self.server_addr[0])
        port = configReader.c_server["port2"]
        try:
            self.server_socket.connect((self.server_addr[0], port))
            return "Port " + port + " is open on addr " + self.server_addr[0]
        except:
            return "Port " + port + " is closed on addr " + self.server_addr[0]
